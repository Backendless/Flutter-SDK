...
class UnitOfWorkUpdate {
  ...
  OpResult update<T>(T changes, [dynamic identifier ]) {
    if (changes is Map) {
      if (identifier is String)
        return _updateMapInstance(identifier, changes);
      if (identifier is OpResult)
        return _updateOpResult(identifier, changes);
      if (identifier is OpResultValueReference)
        return _updateValueRef(identifier, changes);
      throw ArgumentError("The identifier should either tableName, OpResult or OpResultValueReference");
    } else {
  ...
  OpResult _updateOpResult( OpResult result, Map changes )
  {
    ...
    changes["objectId"] = result.resolveTo( propName: "objectId" ).makeReference() ;
  ...
  OpResult _updateValueRef( OpResultValueReference result, Map changes )
  {
    ...
    if( OperationTypeExt.supportCollectionEntityDescriptionType.contains( result.opResult.operationType ) )
      changes["objectId"] = result.resolveTo( "objectId" ).makeReference() ;
    else if( OperationTypeExt.supportListIdsResultType.contains( result.opResult.operationType ) )
