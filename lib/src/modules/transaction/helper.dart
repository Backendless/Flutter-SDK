...
class TransactionHelper {
  ...
  static void makeReferenceToValueFromOpResult( Map<String, dynamic> map )
  {
    map.forEach((key, value) {
      if (value is OpResult) {
        if (OperationTypeExt.supportIntResultType.contains(value.operationType)) {
          value = value.makeReference();
        } else {
          throw ArgumentError("OpResult/OpResultValueReference from this operation in this place not supported");
        }
  ...
  {
    return OperationTypeExt.supportEntityDescriptionResultType.contains( reference.opResult.operationType ) &&
